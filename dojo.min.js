...
var k=ajaxRequest.responseText;if(typeof(e.js_object)!="undefined"){if(e.js_object){k=ajaxRequest.getResponseHeader("Content-Type").indexOf("xml")!=-1?h.xmlToObject(k,true):JSON.parse(k)}}if(c){k={status:"success",results:k,}}e.callback(k)}else{if(ajaxRequest.status!=200){clearTimeout(a);e.callback({status:"error",request_info:ajaxRequest})}}}}},capitalize:function(a){return a.charAt(0).toUpperCase()+a.slice(1)},clickthru:function(a){this.dojo_track({type:"click",key:a.name,});if(a.prepend){a.url=a.prepend+encodeURIComponent(a.url)
}console.log("opening "+a.url);window.open(a.url,"_blank")},contract:function(){if(this.videoPlaying){this.video_close()}if(this.isMraid){mraid.close()}this.dojo_track({type:"interaction",key:"contract",});this.closeCallback()},dojo_track:function(b){if(!this.isDojo||this.dojoConsoleLog){console.log(b.key)}if(this.isDojo){var a=this.dojoUrl+"rmstat?pl="+this.pl+"&adunit="+this.unitID+"&type="+b.type+"&key="+b.key+"&time="+new Date().getTime();this.image_tracker(a)}},expand:function(a){this.dojo_track({type:"interaction",key:"expand",});
if(this.isMraid){this.adIsExpanded=true}},geolocation:function(a){console.log(a);var b={url:this.webServiceUrl+"geolocation/export",method:"GET",callback:a.callback,js_object:true,};if(typeof(a.data)!="undefined"){b.data=a.data}this.ajax(b)},geolocation_prompt:function(c){var a=false;navigator.geolocation.getCurrentPosition(d,b,{enableHighAccuracy:true,timeout:7000,maximumAge:0});function d(e){if(a){return}else{a=true}c.callback({lat:e.coords.latitude,lng:e.coords.longitude})}function b(f){if(a){return
}else{a=true}if(c.failover){dojo.geolocation(c)}else{c.callback(false)}}setTimeout(b,8000)},get_stores:function(b){var c={url:this.webServiceUrl+"phluant/export",method:"GET",callback:b.callback,js_object:true,data:{limit:3,dist:30}};if(typeof(b.data.campaign_id)=="undefined"){console.log("campaign_id is a required attribute for this function");return false}for(var a in b.data){c.data[a]=b.data[a]}c.data.type="get_stores";this.ajax(c)},gid:function(a){return document.getElementById(a)},gmaps_draw:function(c){if(c.map_id===undefined){return console.log("A map id must be specified")
}if(typeof(c.map_id)==="string"){c.map_id=this.gid(c.map_id)}var b=new google.maps.Map(c.map_id,{zoom:c.map_zoom,center:new google.maps.LatLng(c.center_lat,c.center_lng),disableDefaultUI:true,mapTypeId:google.maps.MapTypeId.ROADMAP});var a=new google.maps.LatLngBounds();if(c.markers){c.markers.forEach(function(d){d.position=new google.maps.LatLng(d.lat,d.lng);d.map=b;var e=new google.maps.Marker(d);if(d.clickthru){google.maps.event.addListener(e,"click",function(){if(d.clickthru.callback){d.clickthru.callback.call(this)
}if(!d.clickthru.url){d.clickthru.url="https://maps.google.com/?saddr="+c.user_lat+","+c.user_lng+"&daddr="+this.position.lat()+","+this.position.lng()}dojo.clickthru(d.clickthru)})}if(d.events){for(var f in d.events){google.maps.event.addListener(e,f,d.events[f])}}a.extend(e.position)})}if(c.map_zoom===undefined){b.fitBounds(a)}},gmaps_geo:function(c){if(!c.callback){return false}if(!c.loc_type){c.loc_type="address"}if(!this.geocoder){this.geocoder=new google.maps.Geocoder()}var a={};if(["geo","latLng"].indexOf(c.loc_type)!==-1){if(!this.valid_geo(c.address)){return console.error("invalid lat/lng")
}var b=c.address.split(",");a.latLng=new google.maps.LatLng(b[0],b[1])}else{a.address=encodeURIComponent(c.address)}this.geocoder.geocode(a,function(e,d){dojo.gmaps_return(e,d,c)})},gmaps_return:function(b,a,d){if(a===google.maps.GeocoderStatus.OK){return d.callback(b)}else{if(d.failover){var c={data:{value:d.address},callback:d.failover_callback||d.callback,};if(this.valid_zip(d.address)){c.data.type="postal_code"}else{if(this.valid_geo(d.address)){c.data.type="city_postal_by_geo"}else{return console.error("invalid address")
}}this.geolocation(c)}else{d.callback(false)}}},image_tracker:function(b){var a=document.createElement("img");a.src=b;a.height="1px";a.width="1px";document.getElementsByTagName("body")[0].appendChild(a)},init:function(e){var j=this;if(typeof(e.callback)=="function"){this.closeCallback=e.callback}if(typeof(e.init)=="function"){this.adInit=e.init}if(typeof(e.expanded)!="undefined"){if(e.expanded){j.adIsExpanded=true}}if(typeof(mraid)!="undefined"){this.isMraid=true;mraid.setExpandProperties({useCustomClose:true});
mraid.addEventListener("stateChange",function(){if(j.adIsExpanded){if(typeof(j.closeCallback)!="null"){j.closeCallback()}j.adIsExpanded=false}});var a=document.body;if(typeof(e.tag_elem)!="undefined"){a=e.tag_elem;if(typeof(a)=="string"){a=this.gid(a)}}document.body.style.margin="0px";a.style.position="absolute";var h=document.createElement("meta");h.name="viewport";h.content="width=device-width, minimum-scale=1.0, maximum-scale=1.0";document.getElementsByTagName("head")[0].appendChild(h)}if(typeof(e.asynch_load)!="undefined"){var b=e.asynch_load.scripts;
var l=e.asynch_load.insert_before;var f=b.length;var d=0;if(typeof(l)=="string"){l=this.gid(l)}var c=l.parentNode;for(var k=0;k<b.length;k++){var g=document.createElement("script");g.src=b[k];c.insertBefore(g,l);g.onload=function(){d++;if(d==f){j.init_check()}}}}else{this.init_check()}},init_check:function(){if(this.adInit!=null){if(this.isMraid){if(mraid.getState()==="loading"){mraid.addEventListener("ready",this.mraid_ready)}else{this.mraid_ready()}}else{this.adInit()}}},iosVersionCheck:function(){var a=window.navigator.userAgent,b=a.indexOf("OS ");
...
if(c.indexOf("?")!=-1){var f=new Object();var b=c.split("?");var g=b[1].split("&");for(var d=0;d<g.length;d++){var a=g[d].split("=");f[a[0]]=decodeURIComponent(a[1])}if(e){f=JSON.stringify(f)}return f}else{return false}},roundIt:function(b,a){var c=1;if(a>0){c=Math.pow(10,a)}return Math.round(b*c)/c},shoplocal:function(l){var d={storeid:null,listingcount:50,resultset:"full",sortby:6,listingimagewidth:60,};for(var q in d){if(typeof(l.data[q])!="undefined"){d[q]=l.data[q]}}var j=null;var e={};var p=this;
if(typeof(l.data.call_type)!="undefined"){j=l.data.call_type.split(",");for(var h=0;h<j.length;h++){if(j[h]!="store"){if(typeof(l.data[j[h]+"ids"])=="undefined"){console.error("data."+j[h]+"ids must be defined for the respective lookup.");return false}else{e[j[h]]={};var r=l.data[j[h]+"ids"].split(",");for(var o=0;o<r.length;o++){e[j[h]][r[o]]=null}}}}}var m=["company","campaignid"];for(var h=0;h<m.length;h++){if(typeof(l.data[m[h]])=="undefined"){console.error("data."+m[h]+" is a required attribute for the shoplocal function.");
return false}}var n=null;var g={callback:f,};if(typeof(l.data.location)!="undefined"){if(pcf.valid_zip(l.data.location)){n=l.data.location;a()}else{if(pcf.valid_geo(l.data.location)){g.data={type:"city_postal_by_geo",value:l.data.location}}else{console.error("Invalid location format.  Must be either a US postal code or lat/lng coordinates.  See documentation for more details.");return false}}}else{pcf.geolocation(g)}function a(){if(d.storeid==null){b()}else{if(j!=null){var v=true;for(var u=0;u<j.length;
u++){for(var s in e[j[u]]){if(e[j[u]][s]==null){k(j[u],s);v=false;break}}}if(v){c()}}else{c()}}}function b(){var s={url:"http://api.shoplocal.com/"+l.data.company+"/2012.1/xml/getstores.aspx?campaignid="+l.data.campaignid+"&citystatezip="+n,method:"GET",yql:{format:"xml",},callback:function(y){if(y.status!="error"){var x=p.xmlToObject(y.results,true);var w=x.query.results.content;var u=w.collection.data;d.storeid=null;for(var v in u){var t=true;if(typeof(l.data.name_flag)!="undefined"&&u[v]["@attributes"].name.indexOf(l.data.name_flag)!=-1){t=false
...
f[k]=[];f[k].push(c)}f[k].push(this.xmlToObject(l,false))}}}return f},};dojo.iosVersion=dojo.iosVersionCheck();if(typeof(global_ad_id1)!="undefined"){var newUrl=global_ad_id1[0].url.split("?");var metricsArray=newUrl[1].split("&");var metrics={};for(var i=0;i<metricsArray.length;i++){var metricParams=metricsArray[i].split("=");metrics[metricParams[0]]=decodeURIComponent(metricParams[1])}dojo.unitID=metrics.ad_id;var scripts=document.getElementsByTagName("script");for(var i=0;i<scripts.length;i++){var scriptSrc=scripts[i].src;
if(scriptSrc.indexOf("dojo.phluant.com")!=-1){scriptSrc=scriptSrc.replace("http://dojo.phluant.com/adj/","");scriptSrc=scriptSrc.split("/");dojo.pl=scriptSrc[0];break}}dojo.isDojo=true};