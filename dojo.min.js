...
if(c){k={status:"success",results:k,}}if(typeof(e.js_object)!="undefined"){if(e.js_object){k=ajaxRequest.getResponseHeader("Content-Type").indexOf("xml")!=-1?h.xmlToObject(k,true):JSON.parse(k)}}if(typeof(e.callback)!="undefined"){e.callback(k)}}else{if(typeof(e.callback)!="undefined"){e.callback({status:"error",request_info:ajaxRequest})}}}},capitalize:function(a){return a.charAt(0).toUpperCase()+a.slice(1)},clickthru:function(a){this.dojo_track({type:"click",key:a.name,});window.open(a.url,"_blank")
...
}if(this.valid_zip(address)){b.data.type="postal_code"}if(this.valid_geo(address)){b.data.type="city_postal_by_geo"}if(typeof(b.data.type)!="undefined"){b.data.value=d.address}this.geolocation(b)}}else{d.callback(false)}}},image_tracker:function(b){var a=document.createElement("img");a.src=b;a.height="1px";a.width="1px";document.getElementsByTagName("body")[0].appendChild(a)},init:function(c){var b=this;if(typeof(c.callback)=="function"){this.closeCallback=c.callback}if(typeof(c.init)=="function"){this.adInit=c.init
...
b=b.split(",");if(typeof(b)=="object"){for(var a=0;a<b.length;a++){if(isNaN(b[a])){passed=false;break}}}else{passed=false}return passed},valid_phone:function(a){if(a){a=a.replace(/[^0-9]/g,"");return/^\d{10}/.test(zipTest)}},valid_zip:function(a){return/^\d{5}(-\d{4})?$/.test(a)},video:function(f){var c=this;this.videoPlaying=true;this.videoId=f.container_id;if(typeof(this.videoId)=="string"){this.videoId=this.gid(this.videoId)}objCheck=["style","attributes"];if(!this.videoReload){for(var e in f){if(objCheck.indexOf(e)!=-1){for(var b in f[e]){this.video_properties[e][b]=f[e][b]
}}else{this.video_properties[e]=f[e]}}}if(this.video_properties.full_screen){this.video_properties.attributes["webkit-playsinline"]=false}if(typeof(this.video_properties.container_id)=="string"){this.video_properties.container_id=this.gid(this.video_properties.container_id)}var g=this.video_properties.container_id;var a=this.video_properties.aspect_ratio.split(":");if(this.video_properties.style.width=="0px"&&this.video_properties.style.height=="0px"){console.log("At least a height or a width for the video element or its parent element must be declared");
...
for(var e in this.video_properties.attributes){dojo_videoElement.setAttribute(e,this.video_properties.attributes[e])}for(var e in this.video_properties.style){dojo_videoElement.style[e]=this.video_properties.style[e]}dojo_videoElement.addEventListener("play",function(){c.dojo_track({type:"video",key:"play"});if(c.video_properties.full_screen){c.video_full_screen(dojo_videoElement)}if(typeof(c.video_properties.play_callback)=="function"){c.video_properties.play_callback()}});dojo_videoElement.addEventListener("pause",function(){if(typeof(c.video_properties.pause_callback)=="function"){c.video_properties.pause_callback()
}});dojo_videoElement.addEventListener("loadedmetadata",function(){var j={"25":false,"50":false,"75":false,};var h=dojo_videoElement.duration;c.videoInt=setInterval(function(){var k=c.roundIt(((dojo_videoElement.currentTime/h)*100),0);for(var l in j){if(k==l&&!j[l]){c.dojo_track({type:"video",key:"quartile"+l});j[l]=true}}},100)});if(this.video_properties.attributes.autoplay===true){setTimeout(function(){dojo_videoElement.play()},500)}if(!this.video_properties.full_screen){dojo_videoElement.addEventListener("ended",function(){c.video_close()
...
var c="endfullscreen";if(b.requestFullscreen){b.requestFullscreen()}else{if(b.mozRequestFullScreen){b.mozRequestFullScreen();c="mozendfullscreen"}else{if(b.webkitRequestFullscreen){b.webkitRequestFullscreen();c="webkitendfullscreen"}else{if(b.msRequestFullscreen){b.msRequestFullscreen();c="msendfullscreen"}}}}b.addEventListener(c,function(){a.video_close()})},xmlToObject:function(h,d){if(d){var a=new DOMParser();h=a.parseFromString(h,"text/xml")}var f={};if(h.nodeType==1){if(h.attributes.length>0){f["@attributes"]={};
for(var e=0;e<h.attributes.length;e++){var b=h.attributes.item(e);f["@attributes"][b.nodeName]=b.nodeValue}}}else{if(h.nodeType==3){f=h.nodeValue}}if(h.hasChildNodes()){for(var g=0;g<h.childNodes.length;g++){var l=h.childNodes.item(g);var k=l.nodeName;if(typeof(f[k])=="undefined"){f[k]=this.xmlToObject(l,false)}else{if(typeof(f[k].push)=="undefined"){var c=f[k];f[k]=[];f[k].push(c)}f[k].push(this.xmlToObject(l,false))}}}return f},};dojo.iosVersion=dojo.iosVersionCheck();var metas=document.getElementsByTagName("meta");
for(var i=0;i<metas.length;i++){if(metas[i].getAttribute("name")=="apple-mobile-web-app-status-bar-style"){dojo.isDojo=true}};