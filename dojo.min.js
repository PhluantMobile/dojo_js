...
},mraid_ready:function(){if(mraid.isViewable()){this.mraid_view_change()}else{mraid.addEventListener("viewableChange",this.mraid_view_change)}},mraid_view_change:function(){if(mraid.isViewable()){if(!this.winLoaded){winLoaded=true;window.dispatchEvent(new Event("load"))}this.track("viewableChange");setTimeout(this.adInit.bind(this))}},query_string:function(e){var c=window.location.href;if(c.indexOf("?")!=-1){var f=new Object();var b=c.split("?");var g=b[1].split("&");for(var d=0;d<g.length;d++){var a=g[d].split("=");
f[a[0]]=decodeURIComponent(a[1])}if(e){f=JSON.stringify(f)}return f}else{return false}},roundIt:function(b,a){var c=1;if(a>0){c=Math.pow(10,a)}return Math.round(b*c)/c},shoplocal:function(l){var d={storeid:null,listingcount:50,resultset:"full",sortby:6,listingimagewidth:60,};for(var q in d){if(typeof(l.data[q])!="undefined"){d[q]=l.data[q]}}var j=null;var e={};var p=this;if(typeof(l.data.call_type)!="undefined"){j=l.data.call_type.split(",");for(var h=0;h<j.length;h++){if(j[h]!="store"){if(typeof(l.data[j[h]+"ids"])=="undefined"){console.error("data."+j[h]+"ids must be defined for the respective lookup.");
