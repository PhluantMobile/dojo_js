...
var a=new google.maps.LatLngBounds();if(c.markers){c.markers.forEach(function(d){d.position=new google.maps.LatLng(d.lat,d.lng);d.map=b;var e=new google.maps.Marker(d);if(d.clickthru){google.maps.event.addListener(e,"click",function(){if(d.clickthru.callback){d.clickthru.callback()}dojo.dojo_track({type:"click",key:d.clickthru.name,});window.open(d.clickthru.url||"https://maps.google.com/?saddr="+c.user_lat+","+c.user_lng+"&daddr="+this.position.k+","+this.position.A,"_blank")})}if(d.events){for(var f in d.events){google.maps.event.addListener(e,f,d.events[f])
}}a.extend(e.position)})}if(c.map_zoom===undefined){b.fitBounds(a)}},gmaps_geo:function(c){if(!c.callback){return false}if(!c.loc_type){c.loc_type="address"}if(!this.geocoder){this.geocoder=new google.maps.Geocoder()}var a={};if(["geo","latLng"].indexOf(c.loc_type)!==-1){if(!this.valid_geo(c.address)){return console.error("invalid lat/lng")}var b=c.address.split(",");a.latLng=new google.maps.LatLng(b[0],b[1])}else{a.address=encodeURIComponent(c.address)}this.geocoder.geocode(a,function(e,d){dojo.gmaps_return(e,d,c)
})},gmaps_return:function(b,a,d){if(a===google.maps.GeocoderStatus.OK){return d.callback(b)}else{if(d.failover){var c={data:{value:d.address},callback:d.failover_callback||d.callback,};if(this.valid_zip(d.address)){c.data.type="postal_code"}else{if(this.valid_geo(d.address)){c.data.type="city_postal_by_geo"}else{return console.error("invalid address")}}this.geolocation(c)}else{d.callback(false)}}},image_tracker:function(b){var a=document.createElement("img");a.src=b;a.height="1px";a.width="1px";document.getElementsByTagName("body")[0].appendChild(a)
},init:function(e){var j=this;if(typeof(e.callback)=="function"){this.closeCallback=e.callback}if(typeof(e.init)=="function"){this.adInit=e.init}if(typeof(e.expanded)!="undefined"){if(e.expanded){j.adIsExpanded=true}}if(typeof(mraid)!="undefined"){this.isMraid=true;mraid.setExpandProperties({useCustomClose:true});mraid.addEventListener("stateChange",function(){if(j.adIsExpanded){if(typeof(j.closeCallback)!="null"){j.closeCallback()}j.adIsExpanded=false}});var a=document.body;if(typeof(e.tag_elem)!="undefined"){a=e.tag_elem;
if(typeof(a)=="string"){a=this.gid(a)}}document.body.style.margin="0px";a.style.position="absolute";var h=document.createElement("meta");h.name="viewport";h.content="width=device-width, minimum-scale=1.0, maximum-scale=1.0";document.getElementsByTagName("head")[0].appendChild(h)}if(typeof(e.asynch_load)!="undefined"){var b=e.asynch_load.scripts;var l=e.asynch_load.insert_before;var f=b.length;var d=0;if(typeof(l)=="string"){l=this.gid(l)}var c=l.parentNode;for(var k=0;k<b.length;k++){var g=document.createElement("script");
...
}return f}else{return false}},roundIt:function(b,a){var c=1;if(a>0){c=Math.pow(10,a)}return Math.round(b*c)/c},shoplocal:function(l){var d={storeid:null,listingcount:50,resultset:"full",sortby:6,listingimagewidth:60,};for(var q in d){if(typeof(l.data[q])!="undefined"){d[q]=l.data[q]}}var j=null;var e={};var p=this;if(typeof(l.data.call_type)!="undefined"){j=l.data.call_type.split(",");for(var h=0;h<j.length;h++){if(j[h]!="store"){if(typeof(l.data[j[h]+"ids"])=="undefined"){console.error("data."+j[h]+"ids must be defined for the respective lookup.");
